    ...
    if (elem.hasControls) {
        ...
        if (navBtns.length) {
            navBtns.forEach(function (btn) {
                const topStyle = sliderheight * 0.5;
...
function createNav(elem, hasThumbnails) {
    if (rwp.has(elem, 'navItems') && !rwp.isEmpty(elem.navItems)) {
        ...
        const standardNavContainer = navContainer.classList.length === 1 && navContainer.classList.contains('tns-nav'); // Make sure a custom nav container isn't specified
