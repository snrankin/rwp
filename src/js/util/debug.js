...
export function dump(el) {
	...
	return function (root, x) {
		...
		function xpathHasClass(className) {
			return (
				"contains(concat(' ', normalize-space(@class), ' '), ' " + // eslint-disable-line
				className +
				" ')" // eslint-disable-line
			);
		}
		...
		for (i = 0; i < len; ++i) {
			...
			if ('SAMP' == elt.tagName) {
				...
				(a = elt.getAttribute('href')) // eslint-disable-line
			) {
		...
		if (document.evaluate && Array.from && root.children.length > 1) {
			...
			addEventListener(searchInput, 'keyup', function (e) {
				...
				searchInputTimer = setTimeout(function () {
					...
					var xpathResult = document.evaluate('.//span[' + classMatches + '][contains(translate(child::text(), ' + xpathString(searchQuery.toUpperCase()) + ', ' + xpathString(searchQuery.toLowerCase()) + '), ' + xpathString(searchQuery.toLowerCase()) + ')]', root, null, XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
					// eslint-disable-next-line
					while ((node = xpathResult.iterateNext())) state.nodes.push(node); // eslint-disable-line
					showCurrent(state);
				}, 400);
			});
