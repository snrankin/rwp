...
use RWP\Vendor\WPBP\Debug\{QM_Debug, QM_Output};
...
class Debug {
	...
	public function __construct($title = '') {
		if (\class_exists('\\QM_Collectors')) {
			if (!\class_exists(__NAMESPACE__ . '\\QM_Debug')) {
				include 'QM_Debug.php';
			}
			$this->title = $title;
			$this->output = array();
			\QM_Collectors::add(new QM_Debug($this->title, $this));
		}
		/**
	...
	public function load(array $output, \QM_Collectors $collectors) {
		if (!\class_exists(__NAMESPACE__ . '\\QM_Output')) {
			include 'QM_Output.php';
		}
		$id = \strtolower(\str_replace(' ', '-', $this->title));
		if ($collector = $collectors::get($id)) {
			$output[$id] = new QM_Output($collector, $this->output, $this->title);
		}
		return $output;
	}
	/**
