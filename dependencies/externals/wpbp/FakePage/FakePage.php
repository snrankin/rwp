 ...
 *
 * @link      https://coderwall.com/p/fwea7g
 *
 * @package   RWP_Vendor\WPBP\FakePage
 * @since     0.2.0
 * @author    Ohad Raz & Mte90 <mte90net@gmail.com>
 * @copyright 2014-2015
 * @license   GPL-2.0+
 * ========================================================================== */
namespace RWP\Vendor\WPBP\FakePage;
...
class FakePage
{
    ...
    public function fake_page_filter($posts)
    {
        ...
        if (\count($posts) === 0 && (\strtolower($wp->request) === $this->slug || isset($wp->query_vars['page_id']) && $wp->query_vars['page_id'] === $this->slug)) {
            // Create a fake post
            $post                   = new \stdClass();
            $post->ID               = $this->id;
            $post->post_author      = 1;
