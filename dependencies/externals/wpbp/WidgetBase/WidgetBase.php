 ...
 * @license   GPL-2.0+
 * ========================================================================== */
namespace RWP\Vendor\WPBP\WidgetBase;
...
class WidgetBase extends \WP_Widget
{
    ...
    function create_widget($args)
    {
        ...
        $defaults = array('label' => '', 'description' => '', 'slug' => '', 'classname' => '', 'width' => array(), 'height' => array(), 'fields' => array(), 'options' => array());
        ...
        $this->slug = $slug;
        // check options
        $this->options = array('classname' => $this->classname, 'description' => $description);
        if (!empty($options)) {
    ...
    function create_field_taxonomyterm($key, $out = "")
    {
        ...
        foreach ($taxes as $tax) {
            ...
            $taxonomy =  \get_taxonomy($tax);
            $terms =  \get_terms($taxonomy->name, array('orderby' => 'name', 'parent' => 0));
            $out .= '<div class="inright">';
            $out .= '<select id="' .  \esc_attr($key['_id']) . '" name="' .  \esc_attr($key['_name']) . '" ';
            $out .= '> ';
            $selected = isset($key['value']) ? $key['value'] : $key['std'];
