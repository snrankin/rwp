...
class Command
{
    ...
    public function run()
    {
        $opts = \getopt('hi:o:', array('help', 'input:', 'output:', 'dry-run', 'keep-sourcemap', 'keep-sourcemap-comment', 'linebreak-position:', 'memory-limit:', 'pcre-backtrack-limit:', 'pcre-recursion-limit:', 'remove-important-comments'));
        ...
        $this->setStat('compression-time-start', \microtime(\true));
        $css = $cssmin->run($css);
        $this->setStat('compression-time-end', \microtime(\true));
        $this->setStat('peak-memory-usage', \memory_get_peak_usage(\true));
    ...
    protected function showStats()
    {
        ...
        $compressionTime = $this->formatMicroSeconds($this->stats['compression-time-end'] - $this->stats['compression-time-start']);
