...
class Extended_Taxonomy
{
    ...
    public function __construct(string $taxonomy, $object_type, array $args = [], array $names = [])
    {
        ...
        $this->defaults['labels'] = [
            ...
            'no_item' => \sprintf('No %s', $this->tax_singular_low),
            # Custom label
            'filter_by' => \sprintf('Filter by %s', $this->tax_singular_low),
        ];
        # Only set rewrites if we need them
        if (isset($args['public']) && !$args['public']) {
            $this->defaults['rewrite'] = \false;
        } else {
            $this->defaults['rewrite'] = ['slug' => $this->tax_slug, 'with_front' => \false, 'hierarchical' => isset($args['allow_hierarchy']) ? $args['allow_hierarchy'] : $this->defaults['allow_hierarchy']];
        }
    ...
    public function register_taxonomy()
    {
        ...
        $post_types = get_post_types(['query_var' => $query_var]);
        ...
        } elseif (\in_array($query_var, ['type', 'tab'], \true)) {
            \trigger_error(esc_html(\sprintf(
                /* translators: %s: Taxonomy query variable name */
