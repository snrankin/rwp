...
class Extended_CPT_Admin
{
    ...
    public function filters()
    {
        ...
        foreach ($this->args['admin_filters'] as $filter_key => $filter) {
            ...
            if (isset($filter['taxonomy'])) {
                ...
                \printf('<label for="%1$s" class="screen-reader-text">%2$s</label>', esc_attr($id), esc_html($tax->labels->filter_by ?? $tax->labels->singular_name));
                # Output the dropdown:
                wp_dropdown_categories(['show_option_all' => $filter['title'], 'hide_empty' => \false, 'hide_if_empty' => \true, 'hierarchical' => \true, 'show_count' => \false, 'orderby' => 'name', 'selected_cats' => get_query_var($tax->query_var), 'id' => $id, 'name' => $tax->query_var, 'taxonomy' => $filter['taxonomy'], 'walker' => $walker]);
                ...
                if (!isset($filter['label'])) {
                    $filter['label'] = \str_replace(['-', '_'], ' ', $filter['meta_key']);
                ...
                # Output a dropdown:
                wp_dropdown_users(['id' => $id, 'include' => $filter['options'], 'name' => 'author', 'option_none_value' => '0', 'selected' => $value, 'show_option_none' => $filter['title']]);
            }
        }
    }
    /**
    ...
    public function glance_items(array $items) : array
    {
        ...
        $url = add_query_arg(['post_type' => $this->cpt->post_type], admin_url('edit.php'));
...
ICONCSS;
    ...
    public function cols(array $cols) : array
    {
        ...
        foreach ($cols as $id => $title) {
            if (\in_array($id, $keep, \true) && !isset($this->args['admin_cols'][$id])) {
                $new_cols[$id] = $title;
            }
        }
        ...
        foreach (\array_filter($this->args['admin_cols']) as $id => $col) {
            if (\is_string($col) && isset($cols[$col])) {
                ...
                if (\class_exists('RWP\\Vendor\\coauthors_plus')) {
                    $k = 'coauthors';
                } else {
                    $k = 'author';
                }
                ...
                if (isset($col['connection']) && !\function_exists('RWP\\Vendor\\p2p_type')) {
                    continue;
                }
                if (isset($col['title_cb'])) {
                    ...
                    if (isset($col['title_icon'])) {
                        $title = \sprintf('<span class="dashicons %s" aria-hidden="true"></span><span class="screen-reader-text">%s</span>', esc_attr($col['title_icon']), $title);
                    }
                    $new_cols[$id] = $title;
                }
            }
    ...
    public function col(string $col)
    {
        ...
        if (!\in_array($col, $custom_cols, \true)) {
            return;
        }
    ...
    public function col_taxonomy(string $taxonomy, array $args)
    {
        ...
        if (empty($terms)) {
            \printf('<span aria-hidden="true">&#8212;</span><span class="screen-reader-text">%s</span>', esc_html($tax->labels->no_terms));
            return;
        }
        $out = [];
                ...
                switch ($args['link']) {
                    case 'view':
                        if ($tax->public) {
                            ...
                            $out[] = \sprintf('<a href="%1$s">%2$s</a>', esc_url(get_term_link($term)), esc_html($term->name));
                    ...
                    case 'edit':
                        if (current_user_can($tax->cap->edit_terms)) {
                            $out[] = \sprintf('<a href="%1$s">%2$s</a>', esc_url(get_edit_term_link($term->term_id, $taxonomy, $post->post_type)), esc_html($term->name));
                        } else {
                            $out[] = esc_html($term->name);
                        }
                        break;
                    case 'list':
                        $link = add_query_arg(['post_type' => $post->post_type, $taxonomy => $term->slug], admin_url('edit.php'));
                        $out[] = \sprintf('<a href="%1$s">%2$s</a>', esc_url($link), esc_html($term->name));
                        break;
                }
            } else {
                $out[] = esc_html($term->name);
    ...
    public function col_featured_image(string $image_size, array $args)
    {
        ...
        $image_atts = ['style' => esc_attr(\sprintf('width:%1$s;height:%2$s', $width, $height)), 'title' => ''];
    ...
    public function col_connection(string $connection, array $args)
    {
        ...
        if (isset($args['field']) && isset($args['value'])) {
            $meta = ['connected_meta' => [$args['field'] => $args['value']]];
            ...
            if ($args['link']) {
                switch ($args['link']) {
                    ...
                    case 'edit':
                        ...
                        break;
                    case 'list':
                        $link = add_query_arg(\array_merge(['post_type' => $_post->post_type, 'connected_type' => $connection, 'connected_items' => $post->ID], $meta), admin_url('edit.php'));
    ...
    protected function get_item_title(array $item)
    {
        if (isset($item['taxonomy'])) {
            ...
            return \ucwords(\trim(\str_replace(['post_', '_'], ' ', $item['post_field'])));
            ...
            if (!\function_exists('RWP\\Vendor\\p2p_type') || !$this->p2p_connection_exists($item['connection'])) {
                return $fallback;
            }
            ...
            if (isset($ctype->fields[$item['field']]['values'][$item['value']])) {
                if ('' === \trim($ctype->fields[$item['field']]['values'][$item['value']])) {
