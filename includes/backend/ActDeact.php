...
class ActDeact extends Base {
	...
	public function initialize() {
		...
		\register_activation_hook( RWP_PLUGIN_TEXTDOMAIN . '/' . RWP_PLUGIN_TEXTDOMAIN . '.php', array( self::class, 'activate' ) );
		\register_deactivation_hook( RWP_PLUGIN_TEXTDOMAIN . '/' . RWP_PLUGIN_TEXTDOMAIN . '.php', array( self::class, 'deactivate' ) );
	...
	public static function upgrade_procedure() {
		...
		$version = \get_option( 'rwp-version' );

		if ( ! \version_compare( RWP_PLUGIN_VERSION, $version, '>' ) ) {
			return;
		}

		\update_option( 'rwp-version', RWP_PLUGIN_VERSION, true );
		\delete_option( RWP_PLUGIN_TEXTDOMAIN . '_fake-meta' );
	}
