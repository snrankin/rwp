 ...
 *
 * @package   RWP\/includes/internals/PostTypes/PageForPostType.php
 * @version   0.2.0
...
class PageForPostType extends Singleton {
	...
	public function admin_init() {
		...
		\add_settings_section( 'page_for_post_type', \__( 'Pages for post type archives', 'rwp' ), '__return_false', 'reading' );
		foreach ( $cpts as $cpt ) {
			if ( ! $cpt->has_archive ) {
				continue;
			}
	...
	public function cpt_field( $args ) {
		...
		\wp_dropdown_pages( array(
			...
			'selected' => $value, // phpcs:ignore
			'show_option_none' => wp_sprintf( 'Default (/%s/)', $default ), // phpcs:ignore
		) );
	}
	public function action_customize_register( \WP_Customize_Manager $wp_customize ) {
		$cpts = \get_post_types( array(), 'objects' );
		$wp_customize->add_section( 'page_for_post_type', array( 'title' => \__( 'Pages for post type archives', 'rwp' ) ) );
		foreach ( $cpts as $cpt ) {
			if ( ! $cpt->has_archive ) {
				continue;
			}
	...
	public function filter_display_post_states( $post_states, $post ) {
		...
		if ( 'page' === $post_type ) {
			if ( \in_array( $post->ID, $this->get_page_ids() ) ) {
				...
				$post_states[ "page_for_{$post_type}" ] = wp_sprintf( '%1$s archive', $cpts[ $cpt ]->labels->name );
			}
		}
		return $post_states;
	}
	...
	public function update_post_type( $post_type, $args ) {
		...
		// update the global
		$wp_post_types[ $post_type ] = $args; // phpcs:ignore
	}
	/**
	...
	public function filter_wp_nav_menu_objects( $sorted_items, $args ) {
		...
		foreach ( $sorted_items as &$item ) {
			if ( 'post_type' === $item->type && 'page' === $item->object && \intval( $item->object_id ) === \intval( $page_ids[ $object_post_type ] ) ) {
