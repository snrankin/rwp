 ...
 * SVGs
 *
 * @package   RWP\/includes/internals/SVGs.php
 * @since     1.0.1
...
class SVGs extends Singleton {
	...
	public function initialize() {
		$this->sanitizer = new SVG\Sanitizer();
    ...
    protected static function get_dimensions( $svg_path ) {
		...
		$height = 0;
		/**
		 * @var \SimpleXMLElement|false $svg
		 */
	...
	public function check_for_svg( $file ) {
		if ( 'image/svg+xml' === $file['type'] ) {
			if ( ! $this->sanitize( $file['tmp_name'] ) ) {
				$file['error'] = \__( "Sorry, this file couldn't be sanitized so for security reasons wasn't uploaded", 'rwp' );
			}
		}
		return $file;
	}
	...
	protected function sanitize( $file ) {
		 ...
		 */
		$this->sanitizer->setAllowedTags( new SVG\Tags() );
		$this->sanitizer->setAllowedAttrs( new SVG\Attributes() );
	...
	public function fix_admin_preview( $response, $attachment, $meta ) {
		if ( 'image/svg+xml' === $response['mime'] ) {
			...
			$possible_sizes = \apply_filters( 'image_size_names_choose', array( // phpcs:ignore
				'full' => \__( 'Full Size', 'rwp' ),
				'thumbnail' => \__( 'Thumbnail', 'rwp' ),
				'medium' => \__( 'Medium', 'rwp' ),
				'large' => \__( 'Large', 'rwp' ),
			) );
			$sizes = array();
	...
	public function get_image_tag_override( $html, $id, $alt, $title, $align, $size ) {
		...
		if ( 'image/svg+xml' === $mime ) {
			$dimensions = $this->svg_dimensions( \get_attached_file( $id ) );
			if ( \is_array( $size ) ) {
				$width = $size[0];
				$height = $size[1];
