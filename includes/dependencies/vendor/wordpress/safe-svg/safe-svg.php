...
class SafeSVG {
	...
	function __construct() {
		$this->sanitizer = new SafeSVG\Sanitizer();
		$this->sanitizer->minify(\true);
	...
	protected function sanitize($file) {
		 ...
		 */
		$this->sanitizer->setAllowedTags(new SafeSVG\Tags());
		$this->sanitizer->setAllowedAttrs(new SafeSVG\Attributes());
	...
	protected function is_gzipped($contents) {
		if (\function_exists('mb_strpos')) {
			return 0 === \mb_strpos($contents, "\37" . "‹" . "\10");
		} else {
			return 0 === \strpos($contents, "\37" . "‹" . "\10");
		}
	}
	/**
	...
	public function fix_admin_preview($response, $attachment, $meta) {
		if ($response['mime'] == 'image/svg+xml') {
			...
			$possible_sizes = \apply_filters('image_size_names_choose', array('full' => \__('Full Size'), 'thumbnail' => \__('Thumbnail'), 'medium' => \__('Medium'), 'large' => \__('Large')));
			$sizes = array();
			foreach ($possible_sizes as $size => $label) {
				...
				$sizes[$size] = array('height' => \get_option("{$size}_size_w", $default_height), 'width' => \get_option("{$size}_size_h", $default_width), 'url' => $response['url'], 'orientation' => 'portrait');
			}
	...
	function skip_svg_regeneration($metadata, $attachment_id) {
		...
		if ('image/svg+xml' === $mime) {
			...
			$metadata = array('width' => \intval($dimensions['width']), 'height' => \intval($dimensions['height']), 'file' => $relative_path);
	...
	function add_upgrade_link($links) {
		$mylinks = array('<a target="_blank" style="color:#3db634;" href="https://wpsvg.com/?utm_source=plugin-list&utm_medium=upgrade-link&utm_campaign=plugin-list&utm_content=action-link">Upgrade</a>');
	...
	protected function svg_dimensions($svg) {
		...
		return array('width' => $width, 'height' => $height, 'orientation' => $width > $height ? 'landscape' : 'portrait');
	}
	/**
