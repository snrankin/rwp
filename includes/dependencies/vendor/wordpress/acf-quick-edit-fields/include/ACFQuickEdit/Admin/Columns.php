...
class Columns extends Feature
{
    ...
    public function init_fields()
    {
        ...
        if ('post' == $content_kind) {
            ...
            if ($is_active) {
                $cols_filters[] = ['cb' => [$this, 'move_date_to_end'], 'priority' => 11, 'args' => null];
                $displays_filters[] = ['cb' => [$this, 'display_post_field_column'], 'priority' => 10, 'args' => 2];
            } else {
                // we need at least one column for quick/bulk edit
                $cols_filters[] = ['cb' => [$this, 'add_ghost_column'], 'priority' => null, 'args' => null];
            }
            if ($is_sortable) {
                // posts
        ...
        if ($is_active) {
            $cols_filters[] = [
                ...
                'priority' => 1000,
                // we hook in so late because we have to sort the columns when all of them are present
                'args' => null,
            ];
            if ($is_sortable) {
    ...
    private function get_meta_query($by)
    {
        ...
        if (isset($this->fields, $this->fields[$by])) {
            ...
            $meta_query = ['relation' => 'OR', $by => ['key' => $by, 'compare' => 'NOT EXISTS'] + $type_query, ['key' => $by, 'compare' => 'EXISTS'] + $type_query];
        }
        return $meta_query;
    }
    /**
