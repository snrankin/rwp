...
class FileField extends \RWP\Vendor\ACFQuickEdit\Fields\Field
{
    /**
     *	@inheritdoc
     */
    public function render_column($object_id)
    {
        ...
        if (!\is_null($value) && !empty($value) && ($file = get_post($value))) {
            $output .= \sprintf('<a href="%s" class="acf-qef-icon" title="%s">%s</a>', get_edit_post_link($value), esc_html($file->post_title), wp_get_attachment_image($value, [80, 80], \true));
        }
        return $output;
    }
    /**
