...
class LinkField extends Field
{
    ...
    public function render_column($object_id)
    {
        ...
        return \sprintf('<a href="%s"%s>%s</a>', esc_url($value['url']), !empty($value['target']) ? \sprintf(' target="%s"', esc_attr($value['target'])) : '', !empty($value['title']) ? esc_html($value['title']) : esc_html($value['url']));
