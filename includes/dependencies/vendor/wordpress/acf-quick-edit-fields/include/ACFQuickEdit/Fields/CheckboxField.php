...
class CheckboxField extends \RWP\Vendor\ACFQuickEdit\Fields\ChoiceField
{
    /**
     *	@inheritdoc
     */
    public function render_input($input_atts, $is_quickedit = \true)
    {
        ...
        if ($this->acf_field['toggle']) {
            $output .= \sprintf('<li><label><input %s/>%s</label></li>', acf_esc_attr(['class' => 'acf-checkbox-toggle', 'type' => 'checkbox']), esc_html__(' Toggle All', 'acf-quickedit-fields'));
        }
        $input_atts += ['class' => 'acf-quick-edit', 'id' => $this->core->prefix($this->acf_field['key'])];
        ...
        foreach ($this->acf_field['choices'] as $value => $label) {
            $atts = ['data-acf-field-key' => $this->acf_field['key'], 'type' => 'checkbox', 'value' => $value, 'name' => $field_name, 'id' => $this->core->prefix($this->acf_field['key'] . '-' . $value)];
            if (!$is_quickedit) {
        ...
        if ($this->acf_field['allow_custom']) {
            ...
            $output .= '<li><label>';
            $output .= \sprintf('<input %s />', acf_esc_attr(['type' => 'checkbox', 'class' => 'acf-quick-edit custom', 'checked' => 'checked']));
            $output .= \sprintf('<input %s />', acf_esc_attr(['type' => 'text', 'class' => 'acf-quick-edit', 'data-acf-field-key' => $this->acf_field['key'], 'name' => $field_name, 'style' => 'width:initial']));
            $output .= '</label></li>';
            $output .= '</script>';
        }
        return $output;
