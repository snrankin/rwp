...
class Page_For_Post_Type
{
	...
	public function admin_init() {
		...
		foreach ($cpts as $cpt) {
			...
			\add_settings_field($id, $cpt->labels->name, array($this, 'cpt_field'), 'reading', 'page_for_post_type', array('name' => $id, 'post_type' => $cpt, 'value' => $value));
		}
	}
	public function cpt_field($args) {
		...
		\wp_dropdown_pages(array('name' =>  \esc_attr($args['name']), 'id' =>  \esc_attr($args['name'] . '_dropdown'), 'selected' => $value, 'show_option_none' => \sprintf(\__('Default (/%s/)'), $default)));
	}
	public function action_customize_register(\WP_Customize_Manager $wp_customize) {
		$cpts =  \get_post_types(array(), 'objects');
		$wp_customize->add_section('page_for_post_type', array('title' =>  \__('Pages for post type archives', 'pfpt')));
		foreach ($cpts as $cpt) {
			if (!$cpt->has_archive) {
				continue;
			}
			$id = "page_for_{$cpt->name}";
			$wp_customize->add_setting($id, array('type' => 'option', 'capability' => 'manage_options', 'default' => 0, 'sanitize_callback' => array($this, 'validate_field')));
	...
	public function filter_display_post_states($post_states, $post) {
		...
		$cpts =  \get_post_types(array('public' => \true), 'objects');
		if ('page' === $post_type) {
	...
	public function update_post_type($post_type, $args) {
		...
		if (\is_admin() || '' !=  \get_option('permalink_structure')) {
			if ($args->has_archive) {
				$archive_slug = $args->has_archive === \true ? $args->rewrite['slug'] : $args->has_archive;
				...
				if ($args->rewrite['feeds'] && $wp_rewrite->feeds) {
					$feeds = '(' . \trim(\implode('|', $wp_rewrite->feeds)) . ')';
...
\class_alias(__NAMESPACE__ . '\\Page_For_Post_Type', 'Page_For_Post_Type', \false);
if (!\function_exists('RWP\\Vendor\\get_page_for_post_type')) {
	/**
