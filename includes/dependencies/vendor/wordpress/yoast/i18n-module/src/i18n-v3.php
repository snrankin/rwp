...
class Yoast_I18n_v3 {
     ...
     */
    public function __construct($args, $show_translation_box = \true) {
        if (!is_admin()) {
            return;
        }
    ...
    private function get_admin_locale() {
        if (\function_exists('RWP\\Vendor\\get_user_locale')) {
    ...
    private function hide_promo() {
        ...
        if (!$hide_promo) {
            if (\filter_input(\INPUT_GET, 'remove_i18n_promo', \FILTER_VALIDATE_INT) === 1) {
                ...
                set_transient('yoast_i18n_' . $this->project_slug . '_promo_hide', \true);
                $hide_promo = \true;
            }
        }
        return $hide_promo;
 ...
 * then shows them to the user.
 */
\class_alias(__NAMESPACE__ . '\\Yoast_I18n_v3', 'Yoast_I18n_v3', \false);
