...
class Extended_CPT {
    ...
    public function __construct(string $post_type, array $args = [], array $names = []) {
        ...
        if (isset($names['singular'])) {
            ...
            $this->post_singular = \ucwords(\str_replace(['-', '_'], ' ', $post_type));
        }
        if (isset($names['slug'])) {
        ...
        $this->defaults['labels'] = ['name' => $this->post_plural, 'singular_name' => $this->post_singular, 'menu_name' => $this->post_plural, 'name_admin_bar' => $this->post_singular, 'add_new' => 'Add New', 'add_new_item' => \sprintf('Add New %s', $this->post_singular), 'edit_item' => \sprintf('Edit %s', $this->post_singular), 'new_item' => \sprintf('New %s', $this->post_singular), 'view_item' => \sprintf('View %s', $this->post_singular), 'view_items' => \sprintf('View %s', $this->post_plural), 'search_items' => \sprintf('Search %s', $this->post_plural), 'not_found' => \sprintf('No %s found.', $this->post_plural_low), 'not_found_in_trash' => \sprintf('No %s found in trash.', $this->post_plural_low), 'parent_item_colon' => \sprintf('Parent %s:', $this->post_singular), 'all_items' => \sprintf('All %s', $this->post_plural), 'archives' => \sprintf('%s Archives', $this->post_singular), 'attributes' => \sprintf('%s Attributes', $this->post_singular), 'insert_into_item' => \sprintf('Insert into %s', $this->post_singular_low), 'uploaded_to_this_item' => \sprintf('Uploaded to this %s', $this->post_singular_low), 'filter_items_list' => \sprintf('Filter %s list', $this->post_plural_low), 'filter_by_date' => 'Filter by date', 'items_list_navigation' => \sprintf('%s list navigation', $this->post_plural), 'items_list' => \sprintf('%s list', $this->post_plural), 'item_published' => \sprintf('%s published.', $this->post_singular), 'item_published_privately' => \sprintf('%s published privately.', $this->post_singular), 'item_reverted_to_draft' => \sprintf('%s reverted to draft.', $this->post_singular), 'item_scheduled' => \sprintf('%s scheduled.', $this->post_singular), 'item_updated' => \sprintf('%s updated.', $this->post_singular), 'item_link' => \sprintf('%s Link', $this->post_singular), 'item_link_description' => \sprintf('A link to a %s.', $this->post_singular_low)];
        ...
        if (isset($args['public']) && !$args['public']) {
            ...
            $this->defaults['rewrite'] = ['slug' => $this->post_slug, 'with_front' => \false];
        }
    ...
    public function maybe_filter(\WP_Query $wp_query) {
        if (empty($wp_query->query['post_type']) || !\in_array($this->post_type, (array) $wp_query->query['post_type'], \true)) {
            return;
        }
    ...
    public static function get_filter_vars(array $query, array $filters, string $post_type): array {
        ...
        foreach ($filters as $filter_key => $filter) {
            ...
            if (isset($filter['meta_key'])) {
                $meta_query = ['key' => $filter['meta_key'], 'value' => wp_unslash($query[$filter_key])];
            } elseif (isset($filter['meta_search_key'])) {
                $meta_query = ['key' => $filter['meta_search_key'], 'value' => wp_unslash($query[$filter_key]), 'compare' => 'LIKE'];
            } elseif (isset($filter['meta_key_exists'])) {
                $meta_query = ['key' => wp_unslash($query[$filter_key]), 'compare' => 'EXISTS'];
            } elseif (isset($filter['meta_exists'])) {
                $meta_query = ['key' => wp_unslash($query[$filter_key]), 'compare' => 'NOT IN', 'value' => ['', '0', 'false', 'null']];
            } elseif (isset($filter['post_date'])) {
                $date_query = [$filter['post_date'] => wp_unslash($query[$filter_key]), 'inclusive' => \true];
            } else {
                continue;
            }
    ...
    public function register_post_type() {
        ...
        if (empty($existing)) {
            ...
            if (is_wp_error($cpt)) {
                \trigger_error(esc_html($cpt->get_error_message()), \E_USER_ERROR);
            }
        } else {
...
\class_alias(__NAMESPACE__ . '\\Extended_CPT', 'Extended_CPT', \false);
