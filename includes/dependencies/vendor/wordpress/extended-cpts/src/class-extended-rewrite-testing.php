...
abstract class Extended_Rewrite_Testing {
    ...
    public function get_rewrites(array $struct, array $additional): array {
        ...
        $new = [];
        $rules = $wp_rewrite->generate_rewrite_rules($struct['struct'], $struct['ep_mask'], $struct['paged'], $struct['feed'], $struct['forcomments'], $struct['walk_dirs'], $struct['endpoints']);
        $rules = \array_merge($rules, $additional);
        $feedregex = \implode('|', $wp_rewrite->feeds);
        $replace = ['(.+?)' => 'hello', '.+?' => 'hello', '([^/]+)' => 'world', '[^/]+' => 'world', '(?:/([0-9]+))?' => '/456', '([0-9]{4})' => \date('Y'), '[0-9]{4}' => \date('Y'), '([0-9]{1,2})' => \date('m'), '[0-9]{1,2}' => \date('m'), '([0-9]{1,})' => '123', '[0-9]{1,}' => '789', '([0-9]+)' => \date('d'), '[0-9]+' => \date('d'), "({$feedregex})" => \end($wp_rewrite->feeds), '/?' => '/', '$' => ''];
