...
class Extended_Taxonomy_Admin {
    ...
    public function meta_boxes(string $object_type, $object) {
        ...
        if (\in_array($this->taxo->taxonomy, $taxos, \true)) {
            ...
            wp_add_inline_script('wp-edit-post', \sprintf('wp.data.dispatch( "core/edit-post" ).removeEditorPanel( "taxonomy-panel-%s" );', $this->taxo->taxonomy));
...
<?php
    ...
    public function glance_items(array $items): array {
        ...
        $url = add_query_arg(['taxonomy' => $this->taxo->taxonomy, 'post_type' => \reset($taxonomy->object_type)], admin_url('edit-tags.php'));
