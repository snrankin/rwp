 ...
 */
if (!\class_exists('RWP\\Vendor\\WPTT_WebFont_Loader')) {
	/**
	 * Download webfonts locally.
	 */
	class WPTT_WebFont_Loader {
		...
		public function get_local_stylesheet_url() {
			if (!$this->local_stylesheet_url) {
				$this->local_stylesheet_url = \str_replace($this->get_base_path(), $this->get_base_url(), $this->get_local_stylesheet_path());
			}
		...
		public function get_styles() {
			...
			foreach ($files as $remote => $local) {
				$files[$remote] = \str_replace($this->get_base_path(), $this->get_base_url(), $local);
			}
			$this->css = \str_replace(\array_keys($files), \array_values($files), $this->remote_styles);
		...
		public function schedule_cleanup() {
			if (!\is_multisite() || \is_multisite() && \is_main_site()) {
				if (!\wp_next_scheduled('delete_fonts_folder') && !\wp_installing()) {
					\wp_schedule_event(\time(), self::CLEANUP_FREQUENCY, 'delete_fonts_folder');
				}
			}
		}
		/**
		...
		public function delete_fonts_folder() {
			return $this->get_filesystem()->delete($this->get_fonts_folder(), \true);
		}
		/**
		 * Get the filesystem.
		 *
...
if (!\function_exists('RWP\\Vendor\\wptt_get_webfont_styles')) {
	/**
...
if (!\function_exists('RWP\\Vendor\\wptt_get_webfont_url')) {
	/**
