...
namespace RWP\Vendor\Wa72\HtmlPrettymin;

use RWP\Vendor\JSMin\JSMin;
use RWP\Vendor\tubalmartin\CssMin\Minifier as CSSmin;
...
class PrettyMin
{
    ...
    {
        $resolver->setDefaults(['minify_js' => \true, 'minify_css' => \true, 'remove_comments' => \true, 'remove_comments_exeptions' => ['/^\\[if /'], 'keep_whitespace_around' => [
            ...
            'select',
            'textarea',
        ], 'keep_whitespace_in' => ['script', 'style', 'pre'], 'remove_empty_attributes' => ['style', 'class'], 'indent_characters' => "\t"]);
    }
    /**
    ...
    {
        $resolver = new OptionsResolver();
        $resolver->setDefaults(['minify_js' => $this->options['minify_js'], 'minify_css' => $this->options['minify_css'], 'remove_comments' => $this->options['remove_comments'], 'remove_empty_attributes' => $this->options['remove_empty_attributes']]);
    ...
    {
        if (!$this->options['remove_empty_attributes']) {
            return;
        }
