...
use RWP\Vendor\Symfony\Component\Console\Exception\RuntimeException;
...
use RWP\Vendor\Twig\Loader\ArrayLoader;
...
class LintCommand extends Command {
    ...
    protected function configure() {
        $this->setDescription(self::$defaultDescription)->addOption('format', null, InputOption::VALUE_REQUIRED, 'The output format', 'txt')->addOption('show-deprecations', null, InputOption::VALUE_NONE, 'Show deprecations as errors')->addArgument('filename', InputArgument::IS_ARRAY, 'A file, a directory or "-" for reading from STDIN')->setHelp(
            <<<'EOF'
...
EOF
        ...
        if ($showDeprecations) {
            $prevErrorHandler = \set_error_handler(static function ($level, $message, $file, $line) use (&$prevErrorHandler) {
                if (\E_USER_DEPRECATED === $level) {
                    ...
                    throw new Error\Error($message, $templateLine);
                }
    ...
    private function validate(string $template, string $file): array {
        ...
        try {
            $temporaryLoader = new Loader\ArrayLoader([$file => $template]);
    ...
    private function renderException(SymfonyStyle $output, string $template, Error\Error $exception, string $file = null) {
