...
namespace RWP\Vendor\Symfony\Bridge\Twig\Command;

use RWP\Vendor\Symfony\Component\Console\Command\Command;
use RWP\Vendor\Symfony\Component\Console\Exception\InvalidArgumentException;
use RWP\Vendor\Symfony\Component\Console\Formatter\OutputFormatter;
use RWP\Vendor\Symfony\Component\Console\Input\InputArgument;
use RWP\Vendor\Symfony\Component\Console\Input\InputInterface;
use RWP\Vendor\Symfony\Component\Console\Input\InputOption;
...
use RWP\Vendor\Twig\Loader\ChainLoader;
use RWP\Vendor\Twig\Loader\FilesystemLoader;
...
class DebugCommand extends Command {
    ...
    protected function configure() {
        $this->setDefinition([new InputArgument('name', InputArgument::OPTIONAL, 'The template name'), new InputOption('filter', null, InputOption::VALUE_REQUIRED, 'Show details for all entries matching this filter'), new InputOption('format', null, InputOption::VALUE_REQUIRED, 'The output format (text or json)', 'text')])->setDescription(self::$defaultDescription)->setHelp(
            <<<'EOF'
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        if (null !== $name && [] === $this->getFilesystemLoaders()) {
            throw new InvalidArgumentException(\sprintf('Argument "name" not supported, it requires the Twig loader "%s".', Loader\FilesystemLoader::class));
        }
    ...
    private function displayPathsText(SymfonyStyle $io, string $name) {
        ...
        if ($file->valid()) {
            ...
            if ($paths) {
                ...
                if (Loader\FilesystemLoader::MAIN_NAMESPACE !== $namespace) {
    ...
    private function parseTemplateName(string $name, string $default = Loader\FilesystemLoader::MAIN_NAMESPACE): array {
    ...
    private function getFilesystemLoaders(): array {
        ...
        } elseif ($loader instanceof Loader\ChainLoader) {
            foreach ($loader->getLoaders() as $l) {
                if ($l instanceof Loader\FilesystemLoader) {
