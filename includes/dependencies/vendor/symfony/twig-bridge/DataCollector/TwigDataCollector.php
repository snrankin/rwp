...
namespace RWP\Vendor\Symfony\Bridge\Twig\DataCollector;

use RWP\Vendor\Symfony\Component\HttpFoundation\Request;
use RWP\Vendor\Symfony\Component\HttpFoundation\Response;
use RWP\Vendor\Symfony\Component\HttpKernel\DataCollector\DataCollector;
...
use RWP\Vendor\Twig\Profiler\Dumper\HtmlDumper;
...
class TwigDataCollector extends DataCollector implements LateDataCollectorInterface {
    ...
    private $twig;
    private $computed;
    public function __construct(Profiler\Profile $profile, Environment $twig = null) {
    ...
    public function lateCollect() {
        ...
        $templateFinder = function (Profiler\Profile $profile) use (&$templateFinder) {
            if ($profile->isTemplate()) {
                try {
                    $template = $this->twig->load($name = $profile->getName());
                } catch (Error\LoaderError $e) {
                    $template = null;
                }
    ...
    public function getHtmlCallGraph() {
        $dumper = new Profiler\Dumper\HtmlDumper();
    ...
    public function getProfile() {
        if (null === $this->profile) {
            $this->profile = \unserialize($this->data['profile'], ['allowed_classes' => ['Twig_Profiler_Profile', 'RWP\\Vendor\\Twig\\Profiler\\Profile']]);
        }
        return $this->profile;
    }
    ...
    private function computeData(Profiler\Profile $profile) {
