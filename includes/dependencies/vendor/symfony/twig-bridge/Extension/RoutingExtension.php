...
use RWP\Vendor\Symfony\Component\Routing\Generator\UrlGeneratorInterface;
 ...
 */
final class RoutingExtension extends Extension\AbstractExtension {
    private $generator;
     ...
     */
    public function isUrlGenerationSafe(Node\Node $argsNode): array {
        // support named arguments
        $paramsNode = $argsNode->hasNode('parameters') ? $argsNode->getNode('parameters') : ($argsNode->hasNode(1) ? $argsNode->getNode(1) : null);
        if (null === $paramsNode || $paramsNode instanceof Node\Expression\ArrayExpression && \count($paramsNode) <= 2 && (!$paramsNode->hasNode(1) || $paramsNode->getNode(1) instanceof Node\Expression\ConstantExpression)) {
            return ['html'];
        }
        return [];
    }
