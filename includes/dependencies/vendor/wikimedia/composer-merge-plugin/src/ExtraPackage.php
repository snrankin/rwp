...
class ExtraPackage {
	 ...
	 */
	protected function mergeOrDefer($type, array $origin, array $merge, PluginState $state) {
	...
	protected function mergeConstraints(Link $origin, Link $merge, PluginState $state) {
		...
		$newConstraint = $constraintClass::create(array($origin->getConstraint(), $merge->getConstraint()), \true);
	...
	protected function extractReferences(array $requires, array $references) {
		foreach ($requires as $reqName => $reqVersion) {
			...
			if (\preg_match('{^[^,\\s@]+?#([a-f0-9]+)$}', $reqVersion, $match) && $stabilityName === 'dev') {
