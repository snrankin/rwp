...
namespace RWP\Vendor\PressBits\MediaLibrary;

use RWP\Vendor\WP_Post;
/**
...
class ScalableVectorGraphicsDisplay
{
    ...
    public static function adjust_response_for_svg($response, $attachment, $meta)
    {
        ...
        $response['sizes'] = array('full' => array('url' => $response['url'], 'width' => $dimensions->width, 'height' => $dimensions->height, 'orientation' => $dimensions->width > $dimensions->height ? 'landscape' : 'portrait'));
        return $response;
    }
    /**
     * @since 0.1.0
