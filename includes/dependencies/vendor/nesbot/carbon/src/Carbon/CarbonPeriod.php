 ...
 */
namespace RWP\Vendor\Carbon;

use RWP\Vendor\Carbon\Exceptions\InvalidCastException;
use RWP\Vendor\Carbon\Exceptions\InvalidIntervalException;
use RWP\Vendor\Carbon\Exceptions\InvalidPeriodDateException;
use RWP\Vendor\Carbon\Exceptions\InvalidPeriodParameterException;
use RWP\Vendor\Carbon\Exceptions\NotACarbonClassException;
use RWP\Vendor\Carbon\Exceptions\NotAPeriodException;
use RWP\Vendor\Carbon\Exceptions\UnknownGetterException;
use RWP\Vendor\Carbon\Exceptions\UnknownMethodException;
use RWP\Vendor\Carbon\Exceptions\UnreachableException;
use RWP\Vendor\Carbon\Traits\IntervalRounding;
use RWP\Vendor\Carbon\Traits\Mixin;
use RWP\Vendor\Carbon\Traits\Options;
use Closure;
use Countable;
use DateInterval;
use DatePeriod;
...
class CarbonPeriod implements \Iterator, \Countable, \JsonSerializable
{
    ...
    private function getEndFromRecurrences()
    {
        ...
        if ($this->recurrences === \INF) {
            $start = $this->getStartDate();
            return $start < $start->copy()->add($this->getDateInterval()) ? CarbonImmutable::endOfTime() : CarbonImmutable::startOfTime();
        }
        if ($this->filters === [[static::RECURRENCES_FILTER, null]]) {
            return $this->getStartDate()->copy()->add($this->getDateInterval()->times($this->recurrences - ($this->isStartExcluded() ? 0 : 1)));
        }
        return null;
    }
    /**
