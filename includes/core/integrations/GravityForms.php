...
class GravityForms extends Singleton {
	...
	public function initialize() {
		...
		if ( ! function_exists( 'is_plugin_active' ) || ! \is_plugin_active( 'gravityforms/gravityforms.php' ) || ! rwp_get_option( 'modules.bootstrap.gravityforms', false ) ) {
			return;
		}
			...
			case 'html':
				if ( in_array( 'inline-submit', $css_class ) ) {
					...
					if ( ! empty( $field_content ) ) {
						...
						} elseif ( count( $field_input->getIterator() ) >= 2 ) {
