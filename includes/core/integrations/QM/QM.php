...
class QM extends Singleton {
	...
	public function initialize() {
		...
		if ( ! function_exists( 'is_plugin_active' ) || ! is_plugin_active( 'query-monitor/query-monitor.php' ) ) {
			return;
		}
		...
		if ( class_exists( '\\QM_Collectors' ) ) {
			...
			foreach ( $this->panels as $panel ) {
				...
				$collector = rwp()->get_component( __NAMESPACE__ . "\\Collectors\\$panel" );
	...
	public function load( array $output, \QM_Collectors $collectors ) {
		foreach ( $this->panels as $panel ) {
			...
			if ( $collector ) {
				/**
				 * @var Output\Output $output_class
				 */
				$output_class = rwp()->get_component( __NAMESPACE__ . "\\Output\\$panel" );
	...
	public static function get_theme() {
		...
		} elseif ( isset( $_COOKIE['QM_EDITOR_COOKIE'] ) ) {
