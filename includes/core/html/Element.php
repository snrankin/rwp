	...
	public static function add_background( $bg, $parent_elem ) {
		...
		if ( ! blank( $bg ) ) {
			...
			if ( is_string( $bg ) ) {
				...
				} elseif ( rwp_str_starts_with( $bg, array( 'bg-' ) ) ) {
					$bg_elem->add_class( $bg );
				} elseif ( rwp_is_url( $bg ) || rwp_is_relative_url( $bg ) ) {
					if ( $lazysizes ) {
						if ( $srcset ) {
			...
			} elseif ( $bg instanceof Element || rwp_str_is_html( $bg ) && $bg !== $bg_elem ) {
				$bg_elem = $bg;
			}
