 ...
 * RWP AssetLoader
 *
 * @package RWP\/includes/core/Traits/AssetLoader.php
 * @since   0.1.0
 * ========================================================================== */
...
trait Assets {
	...
	/**
	 * The folder the assets reside in
	 * @property string $assets_folder
	 */
	protected $assets_folder = 'assets/';

	/**
	 * Storage for our relative asset path
	...
	/**
	 * Storage for our relative asset path
	 * @property string $assets_uri
	 */
	protected $assets_uri;

	/**
	 * Storage for our relative asset path
	 * @property array|Collection|false
	 */
	protected $manifest;

	function init_assets() {
		$this->assets_uri = $this->url . $this->assets_folder;
		$this->assets_dir = $this->path . $this->assets_folder;
	...
	public function asset_filename($asset, $folder = '', $prefix = true) {
		...
		if ($prefix) {
			$asset = $this->prefix($asset, 'kebab');
		}

		$manifest = $this->manifest;
		if (!empty($folder)) {
			$folder = trailingslashit($folder);
			$asset = rwp_add_prefix($asset, $folder);
		}
		if ($manifest) {
			try {
	 ...
	 */
	public function asset_dir($asset, $folder = '', $prefix = true) {

		$asset = $this->asset_filename($asset, $folder, $prefix);

		$path = $this->assets_dir . $asset;
	...
	public function asset_uri($asset, $folder = '', $prefix = true) {
		...
		if ($this->asset_dir($asset, $folder, $prefix)) {

			return $this->assets_uri . $file;
		} else {
	...
	public function register_script($args = []) {
		...
		if (is_string($src) && !rwp_is_url($src)) { // if the source is not an external url
			$file = $this->asset_dir($src, $folder);
	...
	public function get_plugin_scripts($location = '') {
		...
		$scripts = data_get($this->config, 'assets.scripts');
	...
	public function get_plugin_styles($location = '') {
		...
		$styles = data_get($this->config, 'assets.styles');
